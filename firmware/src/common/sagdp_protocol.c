...
//			zepto_parser_decode_uint( &po, enc_reply_to, SAGDP_LSENT_PID_SIZE );
			...
			{ 
				...
				if ( ( packet_status & SAGDP_P_STATUS_MASK ) ==  SAGDP_P_STATUS_INTERMEDIATE )
				{
					...
					// re-send LSP
					zepto_copy_request_to_response_of_another_handle( MEMORY_HANDLE_SAGDP_LSM, mem_h );
					// TODO: should we reply with the old address?
//					zepto_copy_request_to_response_of_another_handle( MEMORY_HANDLE_SAGDP_LSM_SAOUDP_ADDR, mem_h_addr );
		...
		if ( ( packet_status & SAGDP_P_STATUS_MASK ) != SAGDP_P_STATUS_FIRST )
		{
			...
			if ( isold )
			{
				...
				if ( ( packet_status & SAGDP_P_STATUS_MASK ) ==  SAGDP_P_STATUS_INTERMEDIATE )
				{
					...
					zepto_copy_request_to_response_of_another_handle( MEMORY_HANDLE_SAGDP_LSM, mem_h );
					// reply is with a new address; new address is not stored as the lsm was terminating and is not supposed to be resent by timeout
					SAGDP_ASSERT_NO_SEQUENCE
...
//			zepto_parser_decode_uint( &po, this_chain_id, SAGDP_LSENT_PID_SIZE );
			...
			if ( is_resent )
			{
				...
				// re-send LSP
				zepto_copy_request_to_response_of_another_handle( MEMORY_HANDLE_SAGDP_LSM, mem_h );
				// TODO: should we reply with a new address?
//				zepto_copy_request_to_response_of_another_handle( MEMORY_HANDLE_SAGDP_LSM_SAOUDP_ADDR, mem_h_addr );
				// SAGDP status remains the same
...
//					*timeout = sagdp_data.last_timeout;
				...
				else*/
				{
					...
					ZEPTO_DEBUG_PRINTF_3( "SAGDP OK: state = %d, packet_status = %d (is old)\n", state, packet_status );
					...
					// we send nothing here; sending will be done by timeout
					SAGDP_ASSERT_SEQUENCE_IN_PROGRESS
					return SAGDP_RET_OK;
				}
			}
			...
			if ( !isreply )
			{
				...
				ZEPTO_DEBUG_PRINTF_3( "SAGDP OK: state = %d, packet_status = %d, !isreply\n", state, packet_status );
...
/*				SAGDP_REGISTER_SUBSEQUENT_RESENT
			...
			else
			{
				...
				zepto_parser_free_memory( MEMORY_HANDLE_SAGDP_LSM_SAOUDP_ADDR );
...
//			zepto_parser_decode_uint( &po, this_chain_id, SAGDP_LSENT_PID_SIZE );
			...
			{
				INCREMENT_COUNTER( 43, "handler_sagdp_receive_up(), wait-remote, first, new (ignored)" );
				ZEPTO_DEBUG_PRINTF_3( "SAGDP OK: state = %d, packet_status = %d\n", state, packet_status );
				return SAGDP_RET_OK; // just ignore
			}
		}
